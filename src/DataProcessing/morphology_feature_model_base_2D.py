...
def generate_nx_graph(df_t):
    ...
    for i in range(len(data)):
        G_l.add_node(i, pos=(data[i][0], data[i][1]))
        G_a.add_node(i, pos=(data[i][0], data[i][1]))
    for i in range(len(data)):
...
def polarity_correlation_function(df_t, G_a):
    ...
    # print(len(notnan_mean_curvature_list))
        ...
        for j in list_i:
            for k in list_i:
                if (
                    ...
                    < eq_radius * interaction_radius_fold  # どれかと繋がりがあるか判定
                ):
                    layer_list[i] += 1
                    break
